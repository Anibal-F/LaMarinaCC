...
export default function WhatsAppChatWidget() {
  ...
  const sendMessage = async () => {
    ...
    if (!waId || !text) {
      setError("Captura número (wa_id) y mensaje.");
      return;
    }
    try {
      setSending(true);
      setError("");
      const response = await fetch(`${apiBase}/whatsapp/chat/messages`, {
        method: "POST",
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 right-6 z-[120] h-14 w-14 rounded-full bg-[#25D366] shadow-lg shadow-[#25D366]/40 text-white flex items-center justify-center hover:scale-105 transition-transform"
      >
        ...
        <div className="fixed bottom-24 right-6 z-[119] w-[360px] max-w-[calc(100vw-1.5rem)] h-[70vh] rounded-xl border border-border-dark bg-surface-dark shadow-2xl overflow-hidden flex flex-col">
          ...
          <div className="px-3 py-2 border-b border-border-dark space-y-2">
            <input
              ...
              placeholder="wa_id (ej. 526691234567)"
              value={draftWaId}
