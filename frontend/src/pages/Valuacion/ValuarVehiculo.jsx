...
export default function ValuarVehiculo() {
  ...
  const handleExportQualitasBudget = async () => {
    ...
    try {
      if (!record?.id) {
        throw new Error("No se pudo identificar la orden para exportar.");
      }

      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/valuacion/ordenes/${record.id}/export-qualitas`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      if (!response.ok) {
        const payload = await response.json().catch(() => null);
        throw new Error(payload?.detail || "No se pudo generar el formato Qualitas.");
      }

      const blob = await response.blob();
      const disposition = response.headers.get("content-disposition") || "";
      const matched = disposition.match(/filename=\"?([^\";]+)\"?/i);
      const filename = matched?.[1] || `${record?.reporte_siniestro || "orden"}_presupuesto_qualitas.xlsx`;
